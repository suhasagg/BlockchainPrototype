...
class SocketCommunication(Node):
    ...
    #For different events in p2p network
    ...
    def node_message(self, connected_node, message):
        ...
        if message.messageType == 'DISCOVERY':
            #DISCOVERY Type Message Handler - populates the global peer list
            self.peerDiscoveryHandler.handleMessage(message)
        elif message.messageType == 'TRANSACTION':
            #TRANSACTION Type Message Handler
            transaction = message.data
            self.node.handleTransaction(transaction)
        elif message.messageType == 'BLOCK':
            #BLOCK type of message Handler - Sending block across all the nodes
            block = message.data
            ...
            #BLOCKCHAIN type of message handler - Synchronizing blockchain across all nodes (in case there is height mismatch bug in blockchain and it needs to be resynchronized)
