...
class PeerDiscoveryHandler():
    ...
    #DISCOVERY Type of Message Handling - GLobal peer list getting populated
    def handleMessage(self, message):
        peersSocketConnector = message.senderConnector
        peersPeerList = message.data
        newPeer = True
        #Peer already in the list - Duplicate Discovery type of Message (Peer will be ignored)
        for peer in self.socketCommunication.peers:
            if peer.equals(peersSocketConnector):
                newPeer = False
        if newPeer:
            self.socketCommunication.peers.append(peersSocketConnector)
        #Fresh new peer, it will be added to peer list
